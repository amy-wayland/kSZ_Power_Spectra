...
plt.figure(figsize=(8, 6))
plt.plot(k_arr, pkt_cen(k_arr, a), label=r'$P_{q_\perp, 1}^{\rm (cen)}$', color='crimson', linewidth=2)
#plt.plot(k_arr, pkt1(k_arr, a), label=r'Central with low satellite fraction', color='deepskyblue', linewidth=2)
plt.plot(k_arr, pkt_sat(k_arr, a), label=r'$P_{q_\perp, 1}^{\rm (cen+sats)}$', color='mediumblue', linewidth=2)
plt.xlim(1e-3, 1e1)
plt.xlabel(r'$k \; [h \, \mathrm{Mpc}^{-1}]$', fontsize=28)
plt.ylabel(r'$P_{q_\perp, \, 1}^{\pi T}(k)$', fontsize=28)
plt.loglog()
plt.legend(fontsize=20, frameon=False, ncol=1, loc="lower left")
plt.tick_params(which='both', top=True, right=True, direction='in', width=1, length=5)
...
plt.figure(figsize=(8, 6))
plt.plot(ells, get_Dl(ells, clt_cen), color="crimson", label=r'$D_{\ell}^{\rm (cen)}$', linewidth=2)
#plt.plot(ells, get_Dl(ells, clt1), color="deepskyblue", label=r'Central with low satellite fraction', linewidth=2)
plt.plot(ells, get_Dl(ells, clt_sat), color="mediumblue", label=r'$D_{\ell}^{\rm (cen+sats)}$', linewidth=2)
plt.xlim(2, 1e4)
plt.loglog()
plt.xlabel(r'$\ell$', fontsize=28)
plt.ylabel(r'$[\ell (\ell + 1) \, / \, 2 \pi] \, C_{\ell, \perp, \, 1}^{\pi T}$', fontsize=28)
plt.tick_params(which='both', top=True, right=True, direction='in', width=1, length=5)
plt.legend(fontsize=20, frameon=False, loc="lower right", ncol=1)

...
class _HaloProfileHE_withFT(ccl.halos.HaloProfile):
    ...
    def _form_factor(self, x):
        return (np.log(1+x)/x)**self._gamma
    ...
    def _integ_interp(self):
        ...
        def integrand(x, gamma):
            return self._form_factor_fourier(x) * x
    ...
    def _Ub_fourier(self, cosmo, M, a, k):
        ...
        return np.exp(self._integ_interp(np.log(q)))
    ...
    def _fourier(self, cosmo, k, M, a):
        ...
        Ub_k = np.array([self._Ub_fourier(k_use, rs_i) for rs_i in r_s])
        Ue_k = np.array([self._Ue_fourier(k_use, re_i) for re_i in r_ej])
        
        fb = self._get_fractions(cosmo, M_use)[0]
        fe = self._get_fractions(cosmo, M_use)[1]
        
        prof_k = M[:, None] * a**(-3) * (fb * Ub_k + fe * Ue_k)
